(() => {
  ...
  function toggleTheme() {
    ...
    document.body.style.setProperty('--temp-bg', getComputedStyle(document.documentElement).getPropertyValue('--bg'));
    document.body.style.animation = 'none';
    void document.body.offsetWidth; // trigger reflow
    document.body.style.animation = 'themeTransition 0.6s ease';
  }
  ...
  function playSound() {
    if (el.alarm) {
      ...
      el.alarm.volume = 0.7;
      el.alarm.play().catch(e => console.warn('🔇 Звук заблокирован браузером:', e.message));
    }
  }
  ...
  function showNotificationModal() {
    const isWorkSession = state.currentMode === 'work';
    el.notificationTitle.textContent = isWorkSession 
      ? 'Время работы истекло!' 
      : 'Перерыв закончился!';
    el.notificationMessage.textContent = isWorkSession 
  ...
  function sendNotification() {
    ...
    if (Notification.permission === 'granted') {
      const msg = state.currentMode === 'work' 
        ? '🎉 Время отдыхать!' 
        : '⏰ Пора работать!';
      try {
  ...
  function requestNotifications() {
    ...
    Notification.requestPermission().then(perm => {
      ...
      el.notifBtn.title = perm === 'granted' ? 'Уведомления включены' : 'Уведомления отключены';
    });
  }
  ...
  function bindEvents() {
    ...
    document.addEventListener('keydown', (e) => {
      ...
      if (k === ' ' || k === 'spacebar') {
        ...
        e.preventDefault();
        switchMode('long');
      } else if (k === 'escape' && !el.settingsModal.classList.contains('hidden')) {
        closeSettings();
      } else if (k === 'escape' && !el.notificationModal.classList.contains('hidden')) {
  ...
  // Добавляем CSS для анимации темы (если ещё не добавлено)
  const style = document.createElement('style');
  style.textContent = `
    @keyframes themeTransition {
      0% { background-color: var(--bg); }
      50% { background-color: var(--temp-bg); }
